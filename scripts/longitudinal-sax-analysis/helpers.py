...
def repl_text_with_initial_number(file, repl_text_with_number_cols):
    ...
    for item in repl_text_with_number_cols:
...
def build_quotient_ts(file, quotient_cols, drop=True):
    for item in quotient_cols:
        ...
        for idx, _ in enumerate(nom):
            ...
            file[name] = file.loc[:, nom[idx]] / file.loc[:, denom[
                                                                 idx]]  # TODO -> works after implementing column-transform -> numeric!

    # drop after finish building_quotient (it may be that column is needed later in for-loop)
    if drop:
        for item in quotient_cols:
            cols = [i for i in list(file.columns) if i.startswith(f"{item[0]}_") or i.startswith(f"{item[1]}_")]
